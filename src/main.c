...
void print_help(boolean and_exit)
{
    ...
    printf("\n");
    printf("Example:\n");
    printf("    ./verilog-dot --ast --hierarchy -v -o module-graph.dot my_module.v\n");
    printf("\n");
...
shell_args * parse_args(int argc, char ** argv)
{
    ...
    shell_args * tr = (shell_args*) calloc(1,sizeof(shell_args));
...
int main(int argc, char ** argv)
{
    ...
    if(args -> draw_ast == BOOL_FALSE && 
       args -> module_hierarchy == BOOL_FALSE){
        printf(" this cases, %d / %d\n", args->draw_ast, args->module_hierarchy);
        free(args);
        print_help(BOOL_TRUE);
    }
